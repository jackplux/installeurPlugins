...
if(!empty($_POST)) {
	...
	foreach($_POST['button'] as $plugName => $dummy) {
		...
		if(!$repo = installeurPlugins::getRepository($cache_dir.$repository_xmlfile)) {
			plxMsg::Error($plxPlugin->getLang('L_ERR_CACHE'));
			header('Location: plugin.php?p=installeurPlugins');
			exit;
		}

		# on teste si le fichier distant est dispo
		if(!installeurPlugins::is_RemoteFileExists($repo[$plugName]['file'])) {
		...
		if(!installeurPlugins::downloadRemoteFile($repo[$plugName]['file'], $zipfile)) {
		...
		require_once(PLX_PLUGINS."installeurPlugins/dUnzip2.inc.php");
...
if(!$remote_version = installeurPlugins::getRemoteFileContent($repository_url.$repository_version))
...
if($version=='' OR $version!=$remote_version or !is_file($cache_dir.$repository_xmlfile)) {
	installeurPlugins::downloadRemoteFile($repository_url.$repository_xmlfile, $cache_dir.$repository_xmlfile);
}
...
<form action="plugin.php?p=installeurPlugins" method="post" id="form_installeurPlugins">
